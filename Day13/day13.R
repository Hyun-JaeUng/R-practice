...
library(ggplot2)
head(mpg)
str(mpg) # tibble은 dataframe과 유사, 똑같이 다뤄도됨 
...
str(iris)
iris %>% pull(Species) # dplyr 패키지의 벡터로 만드는 함수 
iris %>% select(Species)
str(iris %>% pull(Species))
str(iris %>% select(Species))
iris %>% select_if(is.numeric) %>% head(3) # select_if()
iris %>% select(-Sepal.Length, -Petal.Length) %>% head(3)

# Select column whose name starts with "Petal"
iris %>% select(starts_with("Petal")) 
iris %>% select(starts_with("petal"))  # 기본적으로 대소문자 구분 x
iris %>% select(starts_with("petal", ignore.case=F))  # 대소문자 구분하게 만드는 옵션
...
# Select columns whose name maches a regular expression -> matches 안에 정규표현식 활용
...
exam %>% summarise(n = n()) # 행 갯수
exam %>% tally() # 행 갯수 동일
exam %>% count() # 행 갯수 동일 

exam %>% summarise(mean_math = mean(math))  # math 평균 산출
mean(exam$math) # 벡터로 리턴됨 
