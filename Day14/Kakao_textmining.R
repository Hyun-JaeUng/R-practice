...
fiveReviewWordList_sort <- as.vector(head(sort(fiveReviewWordList$wordIncreseRate, decreasing = T), 8))
names(fiveReviewWordList_sort) <- c('최고', '유용', '조아', '편리', '수수료', '너무', '간편', '감사')
barplot(fiveReviewWordList_sort, main='긍정적인 의미로 쓰인 단어', xlab='명사', ylab='증감율' )
...
oneReviewWordList_sort <- as.vector(head(sort(oneReviewWordList$wordIncreseRate, decreasing = T), 8))
names(oneReviewWordList_sort) <- c('고객', '이름', '신분증', '본인', '오류', '전화', '인증', '번호')
barplot(oneReviewWordList_sort, main='부정적인 의미로 쓰인 단어', xlab='명사', ylab='증감율')
