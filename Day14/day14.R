...
# join을 활용한 데이터 병합
inner_join(test1, test2, by = "no") # 같은애가 없으면 해당 데이터 뻄
left_join(test1, test2, by = "no") # 왼쪽에 있는 건 모두 포함
right_join(test1, test2, by = "no") # 오른쪽 모두 포함
full_join(test1, test2, by = "no") # 둘 다 없어도 다 포함

merge(test1, test2) # inner_join
merge(test1, test2, all.x = T) # left_join
merge(test1, test2, all.y = T) # right_join
merge(test1, test2, all = T) # full_join
...
View(mpg)
boxplot(mpg$hwy)
boxplot(mpg$hwy, notch=T) # , notch=T , 중간값을 더 눈에 띄게 해줌
boxplot(mpg$hwy, range=0) # 이상치 범위에 있더라도 표시 안하고, 최댓값으로 쭉 늘림
summary(mpg$hwy)
...
# aggregate -> group_by() 랑 동일. 
agg <- aggregate(iris[,-5], by=list(iris$Species), # 그룹단위로 평균냄 
                 FUN=mean)
agg
...
str(mpg)
ggplot(mpg) # 빈 도화지 생성 (그림 그릴 공간이라 생각)
ggplot(mpg,aes(displ, hwy)) # aes -> 에스테틱의 약어, x축과 y축에 대한 정보
ggplot(mpg,aes(displ, hwy)) + geom_point() # + 연산자를 통한 chaining 느낌임. #산점도 그림
...
sketch + geom_line() # 선
sketch + geom_point() + coord_cartesian(xlim=c(3, 6)) # x축 범위 설정, 3에서 6만 그래프 보여줌
...
ggplot(mtcars, aes(x=cyl)) + geom_bar(aes(fill=factor(gear)), alpha=0.5) # 알파는 투명도
...
class(Cars93$Cylinders) # 팩터
class(Cars93$Price) # 수치형
...
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") # geom_col() 과 같은 기능. 
