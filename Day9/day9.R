...
webElem$sendKeysToElement(list("PYTHON", key = "enter")) # 파이썬 검색 자동화
...
#복수형으로 노드 추출 (findElements 사용 -> 리스트로 리턴됨)
...
if(length(more) == 0) 
  cat("<p> 태그는 없슈\n")
sapply(more, function(x) x$getElementTagName()) # 비어있는 리스트 리턴
sapply(more, function(x) x$getElementText())
...
one$getElementTagName()
one$getElementText()
one$clickElement() #클릭을 통해 스스로 페이지 이동 가능함
...
one$getElementTagName()
one$getElementText()
remDr$executeScript("arguments[0].click();",list(one)); # 리스트 객체로 줘야됨!
# 자바스크립트 코드 줘서 클릭이벤트 실행 (위의 clickelement 안먹으면 이 방법 사용)
...
remDr$navigate(url)
Sys.sleep(3) # 에러방지 위해서 navigate하는 시간 주기
pageLink <- NULL
reple <- NULL
curr_PageOldNum <- 0
repeat{
...
booksites <- sapply(booksitenodes, function(x) {x$getElementAttribute("class")}) # 한글이 아닌 class 속성에 있는 영어이름 가져오기 위함
...
for (booksite in booksites) {
  ...
  if (!dir.exists('output/book')) # 디렉토리 존재 확인
    dir.create('output/book') # 디렉토리 생성
  print(df)
...
# [ YES24의 명견만리 댓글 읽어오기 ]
# 소스코드 안에 없으면 정적 크롤링 불가능. -> 동적 스크래핑 사용
# 자바스크립트로 ajax로 만든 것이라 Rselenium 사용하면 됨!

# 스크롤 내리고, 짧은 댓글을 전체 댓글로 만드는게 어려운 포인트
library(RSelenium)
remDr <- remoteDriver(remoteServerAddr = "localhost", port = 4445, browserName = "chrome")
remDr$open()
...
# 페이지 상황에 맞게 능동적인 대처 필요. 
# scrollIntoView, 특정 태그 영역 안에서 스크롤 처리
